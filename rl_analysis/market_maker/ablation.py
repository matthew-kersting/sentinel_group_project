...
ALL_FEATURES = ["spread", "book_imbalance", "ofi_rolling", "volatility",
...
def evaluate_multi(agent_name, test_data, n_episodes, model=None,
                   obs_features=None, random_agent=False):
    records = []
    ...
    for config_name, features in ABLATION_CONFIGS.items():
        ...
        if os.path.exists(model_path + ".zip"):
            ...
            train_env = Monitor(MarketMakingEnv(
                data=train_data, episode_length=EPISODE_LENGTH,
        ...
        df = evaluate_multi(config_name, test_data, N_EVAL_EPISODES,
                            model=model, obs_features=features)
...
def main():
    ...
    all_feat_pnl = ablation_df[ablation_df["agent"] == "all_features"]["total_pnl"].values
    ...
    summary_dict = {
        name: {
            ...
            "win_rate": float((combined[combined["agent"]==name]["total_pnl"] > 0).mean()),
            "mean_abs_inv": float(combined[combined["agent"]==name]["final_inventory"].abs().mean()),
        }
        for name in combined["agent"].unique()
    }
