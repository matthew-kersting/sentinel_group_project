...
from theme import (
    BG, TEAL, BLUE, ORANGE, RED, GRAY, GREEN,
...
REG_UNI   = ["ofi"]
REG_MULTI = ["ofi", "trade_ofi", "cancel_ratio", "n_trades",
             "cancel_rate", "cancel_to_trade_ratio", "mean_lifespan_s"]
CLF_BASE  = ["ofi", "trade_ofi", "cancel_ratio",
             "cancel_rate", "cancel_to_trade_ratio", "mean_lifespan_s"]
CLF_LAG   = ["ofi", "trade_ofi", "cancel_ratio",
             "cancel_rate", "cancel_to_trade_ratio", "mean_lifespan_s",
             "ofi_lag1", "ofi_lag2"]
...
def fit_regression(
    ...
    X_tr_s, X_v_s, X_te_s = scaler.transform(X_tr), scaler.transform(X_v), scaler.transform(X_te)

    model = Ridge(alpha=RIDGE_ALPHA) if "Ridge" in name else LinearRegression()
    model.fit(X_tr_s, y_tr)

    results = {}
    for split_name, X_s, y in [("train", X_tr_s, y_tr), ("val", X_v_s, y_v), ("test", X_te_s, y_te)]:
...
def plot(
    ...
    for j, split_name in enumerate(splits):
        ...
        ax_a.bar(
            x_reg + (j - 1) * width, ic_vals, width,
            color=SPLIT_COLORS[split_name], alpha=0.85,
            label=split_name, edgecolor="white", linewidth=0.5,
        )
    ...
    ax_a.set_title("A  ·  Regression IC by Model", fontsize=FONT_TITLE, fontweight="bold", loc="left")
    ...
    for j, (name, coefs) in enumerate(coef_data.items()):
        ax_b.bar(
            x_coef + (j - 0.5) * width_c, coefs, width_c,
            color=coef_colors[j], alpha=0.85, label=name,
    ...
    ax_b.set_title("B  ·  Regression Coefficients  (M2 vs M3)", fontsize=FONT_TITLE, fontweight="bold", loc="left")
    ...
    for j, split_name in enumerate(splits):
        ...
        ax_c.bar(
            x_clf + (j - 1) * width, auc_vals, width,
            color=SPLIT_COLORS[split_name], alpha=0.85,
            label=split_name, edgecolor="white", linewidth=0.5,
        )
    ...
    ax_c.set_title("C  ·  Classification AUC by Model", fontsize=FONT_TITLE, fontweight="bold", loc="left")
    ax_c.legend(fontsize=FONT_SMALL, frameon=False)
    style_ax(ax_c)

    # ── Panel D: ROC curves on test set ───────────────────────────────────────
    clf_plot_colors = [TEAL, ORANGE]
    for (name, model), scaler, color in zip(clf_models.items(), clf_scalers.values(), clf_plot_colors):
    ...
    ax_d.set_title("D  ·  ROC Curves on Test Set  (active_next)", fontsize=FONT_TITLE, fontweight="bold", loc="left")
...
def main() -> None:
    ...
    for name, feats in [
        ...
        r, m, s = fit_regression(name, feats, train, val, test, y_tr_reg, y_v_reg, y_te_reg)
    ...
    for name, feats in [
        ...
        print(f"Fitting {name}...")
        r, m, s = fit_classification(name, feats, train, val, test, y_tr_clf, y_v_clf, y_te_clf)
