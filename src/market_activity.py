...
def plot(df: pl.DataFrame, out_path: str = "data/output/market_activity.png") -> None:
    ...
    cross = df.filter(pl.col("action") != "R").group_by(["action", "side"]).len()
    actions = ["A", "C", "F", "T"]
    sides = ["B", "A", "N"]
    ...
    # B — intraday activity
    (
        hourly.rename({"len": "message_count"})
        .with_columns(
            pl.col("hour")
    ...
    ax_a.set_xticklabels(
        [d[5:] for d in dates], rotation=45, ha="right", fontsize=FONT_TINY
    )
    ax_a.yaxis.set_major_formatter(millions_formatter())
    ax_a.set_ylabel("Message count", fontsize=FONT_AXIS)
    ax_a.set_title(
        "A  ·  Daily Message Volume", fontsize=FONT_TITLE, fontweight="bold", loc="left"
    )
    style_ax(ax_a)
    ...
    ax_b.legend(
        handles=[
            Patch(color=TEAL, label="Regular session (9:30–4 PM ET)"),
    ...
    ax_c.hist(
        sizes, bins=log_bins, color=SLATE, alpha=0.9, edgecolor="white", linewidth=0.3
    )
    ax_c.set_xscale("log")
    ax_c.set_yscale("log")
    ...
    ax_c.set_title(
        "C  ·  Order Size Distribution  (Add + Trade, ≤5,000 shares)",
        fontsize=FONT_TITLE,
        fontweight="bold",
        loc="left",
    )
    ...
    ax_d.set_ylim(0, 65)
    ax_d.set_title(
        "D  ·  Action × Side  (% of each action type)",
        fontsize=FONT_TITLE,
        fontweight="bold",
        loc="left",
    )
