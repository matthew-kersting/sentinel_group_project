...
for sym, grp in df_feat.groupby('symbol'):
    ...
    label = np.where(ret_future >  THRESHOLD,  1,
            np.where(ret_future < -THRESHOLD, -1, 0))
    grp['label'] = label
...
print("\nLabel distribution:")
print(df_feat['label'].value_counts().rename({1: 'Up(+1)', 0: 'Flat(0)', -1: 'Down(-1)'}))

# ============================================================
# 4. Train / Test Split (chronological, no data leakage)
# ============================================================
FEATURE_COLS = [c for c in df_feat.columns if any(c.startswith(p) for p in
                ['price_mean', 'price_std', 'price_min', 'price_max', 'price_range',
                 'size_sum', 'size_mean', 'ofi_', 'trade_ratio_', 'ret_',
                 'price_dev', 'hour', 'minute', 'session',
                 'side_code', 'action_code', 'size'])]
...
bars = ax.bar(['Down(-1)', 'Flat(0)', 'Up(+1)'],
              label_counts.values,
...
colors_bar = [COLORS[0] if i < 5 else COLORS[2] if i < 10 else COLORS[3]
...
ax.axvline(top20.mean(), color='red', linestyle='--', linewidth=1.2,
...
for i, cls in enumerate(classes):
    ...
    ax.plot(rec, prec, color=COLORS[i], lw=2,
            label=f'{class_labels[cls]}  AP = {ap:.3f}')
...
ax.fill_between(range(0, 7),
                ...
                cv_scores.mean() + cv_scores.std(),
                alpha=0.15, color='red', label=f'+/-1 SD = {cv_scores.std():.4f}')
...
for i, (cls, lbl) in enumerate(class_labels.items()):
    ...
    axes[i].hist(y_pred_prob[:, idx], bins=40, color=COLORS[i],
...
fig, ax = plt.subplots(figsize=(9, 6))
ax.barh(perm_top20.index[::-1], perm_top20.values[::-1],
        xerr=perm_std_top.values[::-1],
        color=COLORS[1], edgecolor='white', capsize=3, linewidth=0.5)
...
for n in n_est_range:
    tmp_rf = RandomForestClassifier(n_estimators=n, max_depth=10,
    ...
    print(f"  n_estimators={n:3d}  train={train_accs[-1]:.4f}  test={test_accs[-1]:.4f}")
...
ax.fill_between(n_est_range, train_accs, test_accs,
                alpha=0.15, color='gray', label='Overfitting Gap')
ax.set_xlabel('n_estimators (Number of Trees)', fontsize=12)
ax.set_ylabel('Accuracy', fontsize=12)
ax.set_title('Hyperparameter Sensitivity: Number of Trees vs. Accuracy',
             fontsize=14, fontweight='bold')
ax.legend()
plt.tight_layout()
...
for d in depth_range:
    tmp_rf = RandomForestClassifier(n_estimators=100, max_depth=d,
                                    class_weight='balanced',
                                    n_jobs=-1, random_state=42)
    tmp_rf.fit(X_train, y_train)
...
ax.set_ylabel('Accuracy', fontsize=12)
ax.set_title('Hyperparameter Sensitivity: Tree Depth vs. Accuracy',
             fontsize=14, fontweight='bold')
ax.legend()
plt.tight_layout()
...
fig, ax = plt.subplots(figsize=(7, 4))
ax.plot(train_sizes, train_scores.mean(axis=1), 'o-', color=COLORS[0], lw=2, label='Train Set')
ax.fill_between(train_sizes,
                train_scores.mean(axis=1) - train_scores.std(axis=1),
                train_scores.mean(axis=1) + train_scores.std(axis=1),
                alpha=0.15, color=COLORS[0])
ax.plot(train_sizes, test_scores.mean(axis=1),  's-', color=COLORS[1], lw=2, label='Validation Set')
ax.fill_between(train_sizes,
