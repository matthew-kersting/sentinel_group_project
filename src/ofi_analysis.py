...
def _print_stats(train: pl.DataFrame) -> None:
    ...
    print(
        f"  OFI p5 / p95     : {np.percentile(ofi, 5):+.3f} / {np.percentile(ofi, 95):+.3f}"
    )
...
def plot(
    ...
    ofi_roll = train.with_columns(
        pl.col("ofi").rolling_mean(window_size=ROLL).alias("ofi_roll")
    )["ofi_roll"].to_numpy()
    ...
    # Panel D: daily IC
    daily_ic = (
        train.with_columns(pl.col("bucket").dt.date().alias("date"))
        .group_by("date")
        .map_groups(
            lambda g: g.with_columns(
                pl.lit(_pearson(g["ofi"].to_numpy(), g["fwd_return"].to_numpy())).alias(
                    "ic"
                )
            )
        )
    ...
    ax_a.hist(
        ofi_arr, bins=80, color=BLUE, alpha=0.85, edgecolor="white", linewidth=0.3
    )
    ...
    ax_a.set_title(
        "A  ·  OFI Distribution", fontsize=FONT_TITLE, fontweight="bold", loc="left"
    )
    ...
    ax_b.plot(
        buckets, ofi_roll, color=TEAL, lw=1.4, label=f"{ROLL}-bucket rolling mean"
    )
    ax_b.axhline(0, color=GRAY, lw=0.8, ls="--", alpha=0.6)
    ax_b.set_ylabel("OFI", fontsize=FONT_AXIS)
    ax_b.set_title(
        "B  ·  OFI over Time  (train period)",
        fontsize=FONT_TITLE,
        fontweight="bold",
        loc="left",
    )
    ...
    ax_c.set_title(
        f"C  ·  OFI Decile vs {HORIZON}-min Forward Return",
        fontsize=FONT_TITLE,
        fontweight="bold",
        loc="left",
    )
    ...
    ax_d.bar(
        ic_dates, ic_vals, color=bar_cols, alpha=0.85, edgecolor="white", linewidth=0.3
    )
    ax_d.axhline(0, color=GRAY, lw=0.8, ls="--", alpha=0.6)
    ax_d.axhline(
        ic_mean, color=ORANGE, lw=1.4, ls="-", label=f"mean IC = {ic_mean:+.3f}"
    )
    ax_d.set_ylabel("Pearson IC  (OFI vs fwd_return)", fontsize=FONT_AXIS)
    ax_d.set_title(
        "D  ·  Daily Information Coefficient",
        fontsize=FONT_TITLE,
        fontweight="bold",
        loc="left",
    )
